...
def draw_bbox(image, bboxes, classes=read_class_names(cfg.YOLO.CLASSES), show_label=True):
    ...
    for i in range(num_boxes[0]):
        ...
        area = (coor[3] - coor[1])*(coor[2] - coor[0])
        if(area<20000):
            continue
        ...
        #print(ycm)
        lista.append([c1,c2,xcm,ycm,area,score,class_ind,bbox_color,bbox_thick])
    #print(out_classes)
    memo = []
    l2 = []
    dista = 999999
        ...
        for j in range(len(lista)):
            if(i != j ):
                if([i,j] not in memo):
                    ...
                    d = ((abs(aux[2]-aux2[2])**2 + abs(aux[3]-aux2[3])**2))
                    #print('i:  %d    j: %d'%(i,j))
                    #print('distance %d'%(d))
                    if(d<dista):
                        dista = d
    ...
    if(len(l2)>0):
        for i in range(len(l2)):
            ...
            aux2 = lista[pos2]
            if(aux[6] != aux2[6]):
                if((aux[6] == 0)and(aux2[6] == 3))or((aux[6] == 3)and(aux2[6]==0)):
                    #print(l2[i])
                    #print(aux[0],aux[1],aux2[0],aux2[1])
                    x,y = aux[0]
                    w,h = aux[1]
                    x3 = 0
                    y3 = 0
                    ...
                    #print(c1,c2)
                    '''
                    if (aux not in remover):
                        remover.append(aux)
                    '''
                    if (aux2 not in remover):
                        remover.append(aux2)
    ...
    for i in range(len(lista)):
        ...
        cv2.rectangle(image, c1, c2, aux[7], bbox_thick)
        #texto1 = str(dista)
        #cv2.putText(image, texto1, (50,570), cv2.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 0), 2, cv2.LINE_AA)
        #cv2.circle(image,(xcm,ycm),5,(0,0,255),-1)
    print(len(lista))
    '''
    return image,lista
